...
import { useCreateEmployeMutation } from "@/store/slices/EmployesApi";
...
const ModalCreateEmployes: React.FC<Props> = ({ open, onCancel }) => {
  const [createEmploye, { isLoading }] = useCreateEmployeMutation();
  //  notification
  ...
  const handleFinish = async (values: EmployesType) => {
    const birthDateMs = dayjs(values.birth_date).valueOf();
    const joinDateMs = dayjs(values.date_join).valueOf();
    ...
    try {
      await createEmploye(newData).unwrap(); // <-- faqat newData
      api.success({ message: "Created successfully" });
      onCancel();
      form.resetFields();
    } catch (error) {
...
export default ModalCreateEmployes;
